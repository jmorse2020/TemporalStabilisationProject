...
classdef motor < handle 
    ...
    properties (Constant, Hidden)
       ...
       STEPPERMOTORCLASSNAME='Thorlabs.MotionControl.KCube.StepperMotorCLI';
       INTEGRATEDSTEPPERMOTORDLL='Thorlabs.MotionControl.IntegratedStepperMotorsUI.dll';
       INTEGRATEDSTEPPERMOTORCLASSNAME='Thorlabs.MotionControl.IntegratedStepperMotors';
       CONTROLDLL='Thorlabs.MotionControl.Controls.dll';
%        APTDLL='Thorlabs.APT.dll';
       ...
       TIMEOUTSETTINGS=70000000;    % Default timeout time for settings change
    ...
    methods
        ...
        function connect(h,serialNo)  % Connect device
            ...
            if ~h.isconnected
                switch(serialNo(1:2))
                    case '26'   % Serial number corresponds to a KST101
                        h.deviceNET=Thorlabs.MotionControl.KCube.StepperMotorCLI.KCubeStepper.CreateKCubeStepper(serialNo);
                    ...
                    case '45'   % Serial number corresponds to a K10CR1 
...
%             updatestatus(h);            % Update status variables from device
        ...
        function updatestatus(h) % Update recorded device parameters in matlab by reading them from the devuce
            h.isconnected=(h.deviceNET.IsConnected());   % update isconncted flag
        ...
        function displayStatus(h)
            var = "Is connected:             " + h.isconnected;
            disp(var)
            var = "Serial Number:            " + h.serialnumber;
            disp(var)
            var = "Controller Name:          " + h.controllername;
            disp(var)
            var = "Controller Description:   " + h.controllerdescription;
            disp(var)
            var = "Stage Name:               " + h.stagename;
            disp(var)
            var = "Acceleration:             " + h.acceleration;
            disp(var)
            var = "Max. Velocity:            " + h.maxvelocity;
            disp(var)
            var = "Min. Velocity:            " + h.minvelocity;
            disp(var)
            var = "Position:                 " + h.position;
            disp(var)
        end
        function position=readposition(h)
            position=System.Decimal.ToDouble(h.deviceNET.Position); % read device position ect
        end
        ...
        function loaddlls() % Load DLLs
            if ~exist(motor.DEVICEMANAGERCLASSNAME,'class')
                 try   % Load in DLLs if not already loaded
                    ...
                    NET.addAssembly([motor.MOTORPATHDEFAULT,motor.INTEGRATEDSTEPPERMOTORDLL]);
                    NET.addAssembly([motor.MOTORPATHDEFAULT,motor.CONTROLDLL]);
%                     NET.addAssembly([motor.MOTORPATHDEFAULT,motor.APTDLL]);                    
